...
void epoll_add_handler(struct epoll_data_handler* handler, uint32_t event_mask)
{
...
void epoll_remove_handler(struct epoll_data_handler* handler)
{
...
void epoll_do_reactor_loop()
{
    ...
    while (1) {
        struct epoll_data_handler* handler;

        epoll_wait(epoll_fd, &current_epoll_event, 1, -1);
        handler = (struct epoll_data_handler*) current_epoll_event.data.ptr;
